...
> **[전제 보강]** 이 문서는 브랜치를 *제품 기능(스레드)* 으로 볼 때의 검토다.
> 브랜치 아이디어의 실제 동기가 **로컬퍼스트 — 다중 기기에서 로컬에 쓰고 나중에 push** 였다면
> 결론이 상당히 달라진다. 그 검토는 [local-first.md](./local-first.md)에 따로 있다.
> 요약하면: 커밋 불변성·클라이언트 생성 ID·부모 링크·톰스톤 삭제는 **양쪽 모두에 필요**하고,
> `refs` 테이블과 브랜치 UI(§4.4, 로드맵 3단계)는 **동기화가 목적이라면 불필요**하다.
